...
<div class="space-y-4">
  <div class="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
    <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
      <div>
        <div class="text-sm text-zinc-500 dark:text-zinc-400">Torneio</div>
        <div class="mt-1 text-xl font-semibold">{{ $tournament->title }}</div>
        <p class="mt-2 text-sm text-zinc-600 dark:text-zinc-300">
          {{ (int)$tournament->question_count }} perguntas • 60s por questão •
          Taxa: <span class="font-semibold text-wine">{{ (int)$tournament->entry_fee }} diamantes</span>.
          <span class="block text-xs text-zinc-500 dark:text-zinc-400 mt-1">
            Áudio: música aleatória + SFX de acerto/erro (public/musicas)
          </span>
        </p>
      </div>
  ...
  <div id="cardGame" class="hidden rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
    <div class="flex items-start justify-between gap-3">
      <div>
        ...
        <div class="mt-1 text-base font-semibold"><span id="progressText">—</span></div>
      </div>

      <div class="text-right">
        <div class="text-sm text-zinc-500 dark:text-zinc-400">Tempo</div>
        <div class="mt-1 text-base font-semibold"><span id="timerText">01:00</span></div>
  ...
  <div id="cardDone" class="hidden rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
    ...
    <div class="mt-1 text-xl font-semibold">Você concluiu sua tentativa</div>

    <div class="mt-3 grid gap-3 sm:grid-cols-3">
      <div class="rounded-2xl border border-zinc-200 p-4 dark:border-zinc-800">
        <div class="text-xs text-zinc-500 dark:text-zinc-400">Acertos</div>
        <div class="mt-1 text-lg font-semibold"><span id="doneCorrect">0</span></div>
      </div>
      <div class="rounded-2xl border border-zinc-200 p-4 dark:border-zinc-800">
        <div class="text-xs text-zinc-500 dark:text-zinc-400">Tempo</div>
        <div class="mt-1 text-lg font-semibold"><span id="doneTime">—</span></div>
      </div>
      <div class="rounded-2xl border border-zinc-200 p-4 dark:border-zinc-800">
        <div class="text-xs text-zinc-500 dark:text-zinc-400">Score</div>
        <div class="mt-1 text-lg font-semibold"><span id="doneScore">—</span></div>
      </div>
    </div>

    <div class="mt-3 rounded-2xl border border-zinc-200 bg-zinc-50 p-4 text-sm text-zinc-700 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-200">
      Resultado salvo. A premiação será processada quando o torneio for finalizado.
    </div>

    <div class="mt-4 flex flex-wrap gap-2">
      <a href="{{ route('user.quiz.torneio.show', $tournament) }}"
        class="inline-flex items-center justify-center rounded-2xl bg-wine px-5 py-3 text-sm font-semibold text-white shadow-sm hover:opacity-95 active:scale-[0.99] transition">
        Voltar ao torneio
      </a>
...
(function () {
  ...
  function pauseBgMusic() { bgAudio.pause(); }
  ...
  sfxAudio.addEventListener('ended', () => resumeBgMusic());

  function setSound(on) {
    state.soundOn = !!on;
    elSound.textContent = on ? 'Som: ON' : 'Som: OFF';
    if (on) resumeBgMusic();
    else { bgAudio.pause(); sfxAudio.pause(); }
  }
  ...
  async function startGame() {
    const data = await post("{{ route('user.quiz.torneio.start', $tournament) }}");
  ...
  async function submitAnswer(letter) {
    ...
    try {
      const data = await post("{{ route('user.quiz.torneio.answer', $tournament) }}", {
      ...
      if (timedOut) showFeedback(false, `Tempo esgotado. Correta: ${correctChoice ?? '-'}`);
      else if (letter === null) showFeedback(false, `Sem resposta. Correta: ${correctChoice ?? '-'}`);
      else if (isCorrect) showFeedback(true, 'Correto!');
      else showFeedback(false, `Incorreto. Correta: ${correctChoice ?? '-'}`);
      ...
      setTimeout(() => {
        if (data.finished) {
          ...
          doneTime.textContent = data.time_ms ? `${Math.floor(data.time_ms/1000)}s` : '—';
          doneScore.textContent = String(data.score ?? '—');
  ...
  async function resetGame() {
    ...
    try { await post("{{ route('user.quiz.torneio.reset', $tournament) }}"); } catch (e) {}
